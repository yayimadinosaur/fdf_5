...
/*   Updated: 2017/08/23 18:52:09 by wfung            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (dir == 0)
		mlx_pixel_put(e->mlx, e->win, next, fixed_pt);
	else if (dir == 1)
		mlx_pixel_put(e->mlx, e->win, fixed_pt, next);
}

static void		draw_right(t_env *e, float fixed_pt, float next)
{
	int		i;
	int		j;
	float	next;
